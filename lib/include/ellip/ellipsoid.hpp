...
public:
  ...
  ellipsoid(Vec& x, double rho) : 
    _n(x.size()), _Ae(Vec(0., _n), _n), _x(x) {
    assert(rho > 0);
...
#include <limits>
template <class Vec>
inline double norm(const Vec& x) { return sqrt((x*x).sum()); }

template <class Enclosing, class Oracle, class Vec>
STATUS ellipsoid_dc_discrete(Enclosing& E, Oracle& P, Vec& best_x, double& best_f, 
		    int max_it=100, double tol=1e-4)
{
  ...
  for (int iter = 1; iter <= max_it; ++iter) {
    ...
    if (status != CUT) return flag;
    if (norm(lx - E.x()) < tol) return flag;
    lx = E.x();
  }
  		      ...
  		      int max_it=100, double tol=1e-4)
{
  ...
  for (int iter = 1; iter <= max_it; ++iter) {
    ...
    if (info._is_feasible) {
      flag = FOUND;
      if (f < best_f) { best_f = f; best_x = lx; }
    } 
    E.update(g);
    if (norm(lx - E.x()) < tol) return flag;
    lx = E.x();
